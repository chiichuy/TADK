...
public class NFC40 extends Activity {
	...
	private NdefMessage createNDFMsg(boolean now) {
		String text = "Beam " + (now ? "NOW" : "ON")
				+ " com.oreilly.demo.android.pa.sensordemo  Beam with"
				+ (AARUSE ? "" : "out") + " AAR Use";
		byte[] mimeBytes = "application/com.oreilly.demo.android.pa.sensordemo"
				.getBytes(Charset.forName("US-ASCII"));
		NdefRecord mimeRecord = new NdefRecord(NdefRecord.TNF_MIME_MEDIA,
				mimeBytes, new byte[0], text.getBytes());

		NdefMessage msg = new NdefMessage(new NdefRecord[] { mimeRecord });

		NdefMessage msgWaAAR = new NdefMessage(
				new NdefRecord[] {
						mimeRecord,
						NdefRecord
								.createApplicationRecord("com.oreilly.demo.android.pa.sensordemo") });
	...
	private void setupView() {
		...
		try {
			if (getIntent() != null
				...
				findViewById(R.id.beamon).setOnClickListener(
						new OnClickListener() {
							@Override
							public void onClick(View v) {
								doBeamOn();
				...
				findViewById(R.id.beamnow).setOnClickListener(
						new OnClickListener() {
							@Override
							public void onClick(View v) {
								doBeamNow();
			...
			findViewById(R.id.tagdata).setVisibility(View.VISIBLE);
			((TextView) findViewById(R.id.tagdata)).setText("ERROR: "
					+ t.toString());
			t.printStackTrace();
		}
	}
	...
	private void analyzeIntent(final Intent intent) {
		...
		((TextView) findViewById(R.id.tagdata)).setText(new String(
				((NdefMessage) msgs[0]).getRecords()[0].getPayload()));
	}
}
