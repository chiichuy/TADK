...
public class NFC233 extends Activity {
		...
		try {
			if (NfcAdapter.getDefaultAdapter(this) != null) {
				NfcAdapter.getDefaultAdapter(this).disableForegroundDispatch(
						this);
				NfcAdapter.getDefaultAdapter(this).disableForegroundNdefPush(
						this);
			}
		} catch (Exception t) {
			t.printStackTrace();
		...
		try {
			if (getIntent() != null
					...
					&& (getIntent().getAction().equals(
							NfcAdapter.ACTION_NDEF_DISCOVERED) || getIntent()
							.getAction().equals(
				...
				((TextView) findViewById(R.id.tagid))
						.setText("NFC not enabled!");
			} else {
				findViewById(R.id.tagtype).setVisibility(View.GONE);
				...
				findViewById(R.id.enablefdispatch).setOnClickListener(
						new OnClickListener() {
							@Override
							public void onClick(View v) {
								enableForegroundDispatchHandler
										.sendEmptyMessage(0);
							}
						});

				findViewById(R.id.enablefpush).setVisibility(View.VISIBLE);
				findViewById(R.id.enablefpush).setOnClickListener(
						new OnClickListener() {
							@Override
							public void onClick(View v) {
			...
			findViewById(R.id.tagwrite).setOnClickListener(
					new OnClickListener() {
						@Override
						public void onClick(View v) {
			...
			((TextView) findViewById(R.id.tagid)).setText("ERROR: "
					+ t.toString());
		}
	}
	...
	private void enableForegroundDispatch() {
		...
		Toast.makeText(getBaseContext(),
				"Foreground Dispatch Enabled! Please scan a tag",
	...
	private void enableForegroundPush() {
		...
		Toast.makeText(
				getBaseContext(),
				"Foreground Push Enabled! Please tap another NFC endabled Android phone",
				Toast.LENGTH_SHORT).show();
	...
	private void writeTag() {
		if (mytag == null) {
			Toast.makeText(this, "No tag available to write to!",
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		if (ndefref == null) {
			Toast.makeText(this, "Tag is not Ndef: NULL", Toast.LENGTH_SHORT)
					.show();
			return;
		}

		if (!ndefref.isWritable()) {
			Toast.makeText(this, "The tag is not writable!", Toast.LENGTH_SHORT)
					.show();
			return;
		}

		EditText what = (EditText) findViewById(R.id.what);
		if (what == null || what.getText() == null
				|| what.getText().toString() == null
				|| what.getText().toString().trim().length() < 1) {
			Toast.makeText(this, "Please input some text to write to the tag.",
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		(new Thread() {
			public void run() {
				try {
					Message.obtain(mgsToaster, 0, "Tag writing attempt started")
							.sendToTarget();
					...
					Message.obtain(mgsToaster, 0, "Tag write successful!")
							.sendToTarget();
				} catch (Exception t) {
					t.printStackTrace();

					Message.obtain(mgsToaster, 0,
							"Tag writing failed! - " + t.getMessage())
							.sendToTarget();
				} finally {
	...
	// From here on down is the same code as in
	// com.oreilly.demo.android.pa.sensordemo.NFC
	private void analyzeIntent(final Intent intent) {
		...
		if (tag != null) {
			...
			if (type != NFCType.UNKNOWN) {
				...
				data = getTagRawData(tag);
		}

		if (datastr != null)
			updateViewInfo(id, type, datastr);
		else
			updateViewInfo(id, type, data);
	}
	...
	private void updateViewInfo(String id, NFCType type, String data) {
		...
		if (type != NFCType.UNKNOWN) {
			...
			((TextView) findViewById(R.id.tagtype)).setText("TagType: "
					+ typestr);
		}

		if (data != null) {
			((TextView) findViewById(R.id.tagdata))
					.setText("TagData:\n" + data);
		}
	}
	...
	private NdefMessage getTagData(final Intent intent) {
		...
		// we will only grab the first msg as we are handling only 1 tag at the
		// moment
		return nmsgs[0];
	}
	...
	private String getSubRecordData(final NdefRecord[] records) {
		...
		for (NdefRecord record : records) {
			if (record.getTnf() == NdefRecord.TNF_WELL_KNOWN) {
				...
				data += "OTHER UNKNOW DATA\n";
		}
		return data;
	}

	private byte[] getTagRawData(final NdefMessage msg) {
		if (msg == null || msg.getRecords().length < 1)
			return null;
	 ...
	 * 
	 * Bit_7 is the Text Encoding Field. * if Bit_7 == 0 the the text is encoded
	 * in UTF-8 else if Bit_7 == 1 then the text is encoded in UTF16 Bit_6 is
	 * currently always 0 (reserved for future use) Bits 5 to 0 are the length
	...
	private String getURI(final byte[] payload) {
		...
		return (prefix != null ? prefix : "")
				+ new String(Arrays.copyOfRange(payload, 1, payload.length));
	}
	...
	private String convertUriPrefix(final byte prefix) {
		if (prefix == (byte) 0x00)
			return "";
		else if (prefix == (byte) 0x01)
			return "http://www.";
		else if (prefix == (byte) 0x02)
			return "https://www.";
		else if (prefix == (byte) 0x03)
			return "http://";
		else if (prefix == (byte) 0x04)
			return "https://";
		else if (prefix == (byte) 0x05)
			return "tel:";
		else if (prefix == (byte) 0x06)
			return "mailto:";
		else if (prefix == (byte) 0x07)
			return "ftp://anonymous:anonymous@";
		else if (prefix == (byte) 0x08)
			return "ftp://ftp.";
		else if (prefix == (byte) 0x09)
			return "ftps://";
		else if (prefix == (byte) 0x0A)
			return "sftp://";
		else if (prefix == (byte) 0x0B)
			return "smb://";
		else if (prefix == (byte) 0x0C)
			return "nfs://";
		else if (prefix == (byte) 0x0D)
			return "ftp://";
		else if (prefix == (byte) 0x0E)
			return "dav://";
		else if (prefix == (byte) 0x0F)
			return "news:";
		else if (prefix == (byte) 0x10)
			return "telnet://";
		else if (prefix == (byte) 0x11)
			return "imap:";
		else if (prefix == (byte) 0x12)
			return "rtsp://";
		else if (prefix == (byte) 0x13)
			return "urn:";
		else if (prefix == (byte) 0x14)
			return "pop:";
		else if (prefix == (byte) 0x15)
			return "sip:";
		else if (prefix == (byte) 0x16)
			return "sips:";
		else if (prefix == (byte) 0x17)
			return "tftp:";
		else if (prefix == (byte) 0x18)
			return "btspp://";
		else if (prefix == (byte) 0x19)
			return "btl2cap://";
		else if (prefix == (byte) 0x1A)
			return "btgoep://";
		else if (prefix == (byte) 0x1B)
			return "tcpobex://";
		else if (prefix == (byte) 0x1C)
			return "irdaobex://";
		else if (prefix == (byte) 0x1D)
			return "file://";
		else if (prefix == (byte) 0x1E)
			return "urn:epc:id:";
		else if (prefix == (byte) 0x1F)
			return "urn:epc:tag:";
		else if (prefix == (byte) 0x20)
			return "urn:epc:pat:";
		else if (prefix == (byte) 0x21)
			return "urn:epc:raw:";
		else if (prefix == (byte) 0x22)
			return "urn:epc:";
		else if (prefix == (byte) 0x23)
			return "urn:nfc:";
		return null;
	}
